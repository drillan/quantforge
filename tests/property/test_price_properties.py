...
class TestPriceProperties:
    ...
    def test_price_bounds(self, s: float, k: float, t: float, r: float, v: float) -> None:
    ...
    @given(
        ...
        v1=st.floats(min_value=0.05, max_value=0.5),
        v2=st.floats(min_value=0.05, max_value=0.5),
    )
    @settings(max_examples=300, deadline=None)
    def test_volatility_monotonicity(self, s: float, k: float, t: float, r: float, v1: float, v2: float) -> None:
        """ボラティリティに対する単調性: σ1 < σ2 => C(σ1) < C(σ2)."""
        assume(v1 != v2)

        price1 = calculate_call_price(s, k, t, r, v1)
    ...
    @settings(max_examples=300, deadline=None)
    def test_strike_monotonicity(self, s: float, k1: float, k2: float, t: float, r: float, v: float) -> None:
        """行使価格に対する単調性: K1 < K2 => C(K1) >= C(K2)."""
        assume(k1 != k2)

        price1 = calculate_call_price(s, k1, t, r, v)
    ...
    @settings(max_examples=300, deadline=None)
    def test_delta_bounds(self, s1: float, s2: float, k: float, t: float, r: float, v: float) -> None:
        """デルタの範囲: 0 <= ΔC/ΔS <= 1."""
        assume(abs(s1 - s2) > 0.01)  # 有意な差

        price1 = calculate_call_price(s1, k, t, r, v)
        price2 = calculate_call_price(s2, k, t, r, v)
    ...
    def test_convexity(self, s: float, k: float, t: float, r: float, v: float) -> None:
        ...
        price_low = calculate_call_price(s_low, k, t, r, v)
        price_mid = calculate_call_price(s_mid, k, t, r, v)
        price_high = calculate_call_price(s_high, k, t, r, v)
    ...
    @settings(max_examples=300, deadline=None)
    def test_interest_rate_monotonicity(self, s: float, k: float, t: float, r1: float, r2: float, v: float) -> None:
        """金利に対する単調性: r1 < r2 => C(r1) <= C(r2)."""
        assume(r1 != r2)

        price1 = calculate_call_price(s, k, t, r1, v)
        price2 = calculate_call_price(s, k, t, r2, v)
...
class TestBatchProperties:
    ...
    @settings(max_examples=200, deadline=None)
    def test_batch_consistency(self, spots: list[float], k: float, t: float, r: float, v: float) -> None:
        """バッチ処理と個別処理の一貫性."""
        spots_array = np.array(spots)

        # バッチ処理
        batch_prices = calculate_call_price_batch(spots_array, k, t, r, v)
...
class TestSpecialCases:
    ...
    @settings(max_examples=200, deadline=None)
    def test_deep_otm_convergence(self, s_ratio: float, k: float, t: float, r: float, v: float) -> None:
        """Deep OTMでゼロへの収束."""
    ...
    @settings(max_examples=200, deadline=None)
    def test_deep_itm_convergence(self, s_ratio: float, k: float, t: float, r: float, v: float) -> None:
        """Deep ITMで本質的価値への収束."""
...
class TestNumericalStability:
    ...
    @settings(max_examples=200, deadline=None)
    def test_scale_invariance(self, scale: float, k: float, t: float, r: float, v: float) -> None:
        """スケール不変性のテスト."""
        # 基準価格
        base_price = calculate_call_price(100.0, k, t, r, v)
        ...
        # スケール後の価格
        scaled_price = calculate_call_price(scaled_s, scaled_k, t, r, v)
