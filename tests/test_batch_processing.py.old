"""Tests for batch processing functions across all black_scholes."""
...
# class TestAmericanBatch: # TODO: Re-enable when American option is implemented
...
#         prices = black_scholes.american.call_price_batch(spots, strikes, times, rates, qs, sigmas)
...
#         prices_broadcast = black_scholes.american.call_price_batch(spots, strikes, times, rates, qs, sigmas)
#         np.testing.assert_array_almost_equal(prices, prices_broadcast)
...
#         prices = black_scholes.american.put_price_batch(spots, strikes, times, rates, qs, sigmas)
...
# Calculate IV for each price
#         ivs = black_scholes.american.implied_volatility_batch(prices, spots, strikes, times, rates, qs, is_calls)
...
# Verify consistency with single calculation
#         single_iv = black_scholes.american.implied_volatility(prices[0], 100.0, 100.0, 1.0, 0.05, 0.03, is_call=False)
#         assert abs(ivs[0] - single_iv) < 1e-10
...
# Calculate Greeks for each spot
#         greeks = black_scholes.american.greeks_batch(spots, strikes, times, rates, qs, sigmas, is_calls)
...
# Verify consistency with single calculation
#         single_greeks = black_scholes.american.greeks(spots[0], 100.0, 1.0, 0.05, 0.03, 0.2, is_call=False)
#         assert abs(greeks["delta"][0] - single_greeks['delta']) < 1e-10
#         assert abs(greeks["gamma"][0] - single_greeks['gamma']) < 1e-10
#         assert abs(greeks["vega"][0] - single_greeks['vega']) < 1e-10
#         assert abs(greeks["theta"][0] - single_greeks['theta']) < 1e-10
...
#         boundaries = black_scholes.american.exercise_boundary_batch(
#             spots, strikes, times, rates, qs, sigmas, is_calls
#         )
...
# Verify consistency with single calculation
#         single_boundary = black_scholes.american.exercise_boundary(
#             spots[0], 100.0, 1.0, 0.05, 0.03, 0.2, is_call=False
#         )
...
class TestScalarInputs:
    ...
    def test_black_scholes_scalar_inputs(self) -> None:
        ...
        # Test Python list input (convert to numpy array)
        result = black_scholes.call_price_batch(np.array([95.0, 100.0, 105.0]), 100.0, 1.0, 0.05, 0.2)
        assert len(result) == 3
        assert all(r > 0 for r in result)
    ...
    def test_black76_scalar_inputs(self) -> None:
        ...
        forwards = np.array([95.0, 100.0, 105.0])
        greeks = black76.greeks_batch(forwards, 100.0, 1.0, 0.05, 0.2, True)
        assert isinstance(greeks, dict)
    ...
    def test_merton_scalar_inputs(self) -> None:
        ...
        # Test Greeks with numpy arrays
        spots = np.array([95.0, 100.0, 105.0])
        greeks = merton.greeks_batch(spots, 100.0, 1.0, 0.05, 0.02, 0.2, True)
        assert isinstance(greeks, dict)
    ...
    def test_american_scalar_inputs(self) -> None:
        ...
        #         result = black_scholes.american.call_price_batch(
        ...
        #         spots = [95, 100, 105]
        # TODO: Re-enable when American option is implemented
        #         greeks = black_scholes.american.greeks_batch(
        #             spots, 100.0, 1.0, 0.05, 0.02, 0.2, 1.0
        #         )
        #         assert isinstance(greeks, dict)
        #         assert "dividend_rho" in greeks
        #         assert len(greeks["dividend_rho"]) == 3
        pass  # Placeholder until American option is implemented

    def test_implied_volatility_scalar_inputs(self) -> None:
        """Test implied volatility with scalar inputs."""
        # Black-Scholes
        iv = black_scholes.implied_volatility_batch(10.0, 100.0, 100.0, 1.0, 0.05, True)
        ...
        ivs = black_scholes.implied_volatility_batch(prices, 100.0, 100.0, 1.0, 0.05, True)
        assert len(ivs) == 3
