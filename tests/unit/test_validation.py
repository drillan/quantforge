...
class TestInputValidation:
    ...
    def test_zero_spot_price(self) -> None:
        """ゼロのスポット価格でエラー."""
        with pytest.raises(ValueError, match="[Ss]pot"):
            calculate_call_price(0.0, 100.0, 1.0, 0.05, 0.2)
    ...
    def test_zero_strike_price(self) -> None:
        """ゼロの行使価格でエラー."""
        with pytest.raises(ValueError, match="[Ss]trike"):
            calculate_call_price(100.0, 0.0, 1.0, 0.05, 0.2)
    ...
    def test_zero_time_to_maturity(self) -> None:
        """ゼロの満期までの時間でエラー."""
        with pytest.raises(ValueError, match="[Tt]ime"):
            calculate_call_price(100.0, 100.0, 0.0, 0.05, 0.2)
    ...
    def test_zero_volatility(self) -> None:
        """ゼロのボラティリティでエラー."""
        with pytest.raises(ValueError, match="[Vv]olatility"):
            calculate_call_price(100.0, 100.0, 1.0, 0.05, 0.0)

    def test_extreme_interest_rates(self) -> None:
        """極端な金利の処理."""
        # 負の金利（許容範囲内）
        price_neg = calculate_call_price(100.0, 100.0, 1.0, -0.05, 0.2)
        assert price_neg > 0, "負の金利で価格が負"

        # 高い金利（許容範囲内）
        price_high = calculate_call_price(100.0, 100.0, 1.0, 0.5, 0.2)
        assert price_high > 0, "高金利で価格が負"

        # 許容範囲外の金利
        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 1.0, 1.5, 0.2)
...
class TestNumericValues:
    ...
    def test_nan_inputs(self) -> None:
        ...
        with pytest.raises(ValueError):
            calculate_call_price(nan, 100.0, 1.0, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, nan, 1.0, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, nan, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 1.0, nan, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 1.0, 0.05, nan)
    ...
    def test_inf_inputs(self) -> None:
        ...
        with pytest.raises(ValueError):
            calculate_call_price(inf, 100.0, 1.0, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, inf, 1.0, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, inf, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 1.0, inf, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 1.0, 0.05, inf)
    ...
    def test_very_small_positive_values(self) -> None:
        ...
        price1 = calculate_call_price(min_price, 100.0, 1.0, 0.05, 0.2)
        assert price1 >= 0, "最小価格で負の結果"

        price2 = calculate_call_price(100.0, min_price, 1.0, 0.05, 0.2)
        assert price2 >= 0, "最小行使価格で負の結果"

        price3 = calculate_call_price(100.0, 100.0, min_time, 0.05, 0.2)
        assert price3 >= 0, "最小時間で負の結果"

        price4 = calculate_call_price(100.0, 100.0, 1.0, 0.05, min_vol)
        assert price4 >= 0, "最小ボラティリティで負の結果"

        # 許容範囲外の小さい値
        with pytest.raises(ValueError):
            calculate_call_price(0.001, 100.0, 1.0, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 0.0001, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 1.0, 0.05, 0.001)
    ...
    def test_very_large_values(self) -> None:
        ...
        # 許容される最大値
        price1 = calculate_call_price(max_price - 1, 100.0, 1.0, 0.05, 0.2)
        assert price1 >= 0, "最大価格で負の結果"
        assert math.isfinite(price1), "最大価格で無限大"

        price2 = calculate_call_price(100.0, max_price - 1, 1.0, 0.05, 0.2)
        assert price2 >= 0, "最大行使価格で負の結果"
        assert math.isfinite(price2), "最大行使価格で無限大"

        price3 = calculate_call_price(100.0, 100.0, max_time, 0.05, 0.2)
        assert price3 >= 0, "最大時間で負の結果"
        assert math.isfinite(price3), "最大時間で無限大"

        price4 = calculate_call_price(100.0, 100.0, 1.0, 0.05, max_vol)
        assert price4 >= 0, "最大ボラティリティで負の結果"
        assert math.isfinite(price4), "最大ボラティリティで無限大"

        # 許容範囲外の大きい値
        with pytest.raises(ValueError):
            calculate_call_price(max_price * 2, 100.0, 1.0, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, max_time * 2, 0.05, 0.2)

        with pytest.raises(ValueError):
            calculate_call_price(100.0, 100.0, 1.0, 0.05, max_vol * 2)
...
class TestBatchValidation:
    ...
    def test_empty_batch(self) -> None:
        """空のバッチ処理."""
        spots = np.array([])
        result = calculate_call_price_batch(spots, 100.0, 1.0, 0.05, 0.2)
    ...
    def test_batch_with_invalid_values(self) -> None:
        ...
        with pytest.raises(ValueError):
            calculate_call_price_batch(spots_nan, 100.0, 1.0, 0.05, 0.2)

        # 負の値を含むバッチ
        spots_neg = np.array([100.0, -50.0, 110.0])
        with pytest.raises(ValueError):
            calculate_call_price_batch(spots_neg, 100.0, 1.0, 0.05, 0.2)

        # ゼロを含むバッチ
        spots_zero = np.array([100.0, 0.0, 110.0])
        with pytest.raises(ValueError):
            calculate_call_price_batch(spots_zero, 100.0, 1.0, 0.05, 0.2)
...
class TestErrorMessages:
    ...
    def test_error_includes_value(self) -> None:
        ...
        with pytest.raises(ValueError) as exc_info:
            calculate_call_price(-123.45, 100.0, 1.0, 0.05, 0.2)
        # エラーメッセージに実際の値が含まれるか
...
class TestBoundaryConditions:
    ...
    def test_minimum_valid_inputs(self) -> None:
        ...
        # すべて最小値
        price = calculate_call_price(min_price, min_price, min_time, min_rate, min_vol)
    ...
    def test_maximum_valid_inputs(self) -> None:
        ...
        # すべて最大値
        price = calculate_call_price(max_price, max_price, max_time, max_rate, max_vol)
    ...
    def test_mixed_boundary_values(self) -> None:
        """境界値の組み合わせテスト."""
        # 最小スポット、最大行使価格
        price1 = calculate_call_price(0.01, 2147483648.0 - 1, 1.0, 0.05, 0.2)
        assert price1 == 0.0, "Deep OTMでゼロでない"

        # 最大スポット、最小行使価格
        price2 = calculate_call_price(2147483648.0 - 1, 0.01, 1.0, 0.05, 0.2)
        assert price2 > 0, "Deep ITMで正でない"

        # 最小時間、最大ボラティリティ
        price3 = calculate_call_price(100.0, 100.0, 0.001, 0.05, 10.0)
