...
class BlackScholesReference:
    ...
    @staticmethod
    def _gbs(
        option_type: str, fs: float, x: float, t: float, r: float, b: float, v: float
        ...
        if option_type == "c":
            ...
            theta = (
                -(fs * v * math.exp((b - r) * t) * norm.pdf(d1)) / (2 * t_sqrt)
    ...
    @staticmethod
    def black_scholes(
        option_type: str, fs: float, x: float, t: float, r: float, v: float
    ) -> tuple[float, float, float, float, float, float]:
    ...
    @staticmethod
    def merton(
        option_type: str, fs: float, x: float, t: float, r: float, q: float, v: float
    ) -> tuple[float, float, float, float, float, float]:
