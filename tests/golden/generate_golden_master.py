...
from gbs_reference.utils import assert_close
...
class GoldenMasterGenerator:
    ...
    def _create_test_case(self, test_id: str, category: str, description: str,
                         s: float, k: float, t: float, r: float, v: float,
        ...
        test_case = {
            ...
            "metadata": {
                "moneyness": moneyness,
                "moneyness_category": self._categorize_moneyness(moneyness)
            }
        }
        
        if q is not None:
            test_case["inputs"]["q"] = q
    ...
    def generate_test_cases(self) -> List[Dict[str, Any]]:
        ...
        rates = [-0.01, 0.0, 0.02, 0.05, 0.1]  # 負金利含む
        ...
        reference_cases = [
            # Rustテストで使用されている値
            {"s": 100.0, "k": 100.0, "t": 1.0, "r": 0.05, "v": 0.2, 
             "desc": "Reference ATM (used in Rust tests)"},
            {"s": 90.0, "k": 100.0, "t": 1.0, "r": 0.05, "v": 0.2,
             "desc": "Reference OTM"},
            {"s": 110.0, "k": 100.0, "t": 1.0, "r": 0.05, "v": 0.2,
             "desc": "Reference ITM"},
        ]
    ...
    def save_golden_values(self, golden_data: Dict[str, Any]):
    ...
    def run(self, force_regenerate: bool = False):
        """ゴールデンマスター生成を実行.
        ...
        if self.is_generated() and not force_regenerate:
            print(f"⚠️  Golden values already generated. Use --regenerate-golden to force regeneration.")
            return
...
def test_generate_golden_master(request):
    """ゴールデンマスター生成テスト."""
