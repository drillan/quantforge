...
import numpy as np
import pytest
from quantforge import models  # type: ignore[import-untyped]
...
def test_put_single_calculation() -> None:
    """単一プット価格計算のテスト"""
    # ATMプット
    price = models.put_price(100.0, 100.0, 1.0, 0.05, 0.2)
    assert abs(price - 5.573526022657734) < PRACTICAL_TOLERANCE

    # ITMプット (S < K)
    itm_price = models.put_price(90.0, 100.0, 1.0, 0.05, 0.2)
    assert itm_price > price  # ITMはATMより高い

    # OTMプット (S > K)
    otm_price = models.put_price(110.0, 100.0, 1.0, 0.05, 0.2)
...
def test_put_call_parity() -> None:
    ...
    call = models.call_price(s, k, t, r, sigma)
    put = models.put_price(s, k, t, r, sigma)
...
def test_put_edge_cases() -> None:
    """エッジケースのテスト"""
    # Deep ITM (S << K)
    deep_itm = models.put_price(10.0, 100.0, 1.0, 0.05, 0.2)
    intrinsic = 100.0 * np.exp(-0.05) - 10.0
    assert deep_itm >= intrinsic * 0.99  # ほぼ内在価値

    # Deep OTM (S >> K)
    deep_otm = models.put_price(500.0, 100.0, 1.0, 0.05, 0.2)
    assert deep_otm < 0.001  # ほぼゼロ

    # 満期直前（最小時間は0.001）
    near_expiry = models.put_price(90.0, 100.0, 0.001, 0.05, 0.2)
...
def test_put_input_validation() -> None:
    ...
    with pytest.raises(ValueError):
        # 負のスポット価格
        models.put_price(-100.0, 100.0, 1.0, 0.05, 0.2)
    ...
    with pytest.raises(ValueError):
        ...
        models.put_price_batch(spots, 100.0, 1.0, 0.05, 0.2)
