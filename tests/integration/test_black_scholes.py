...
class TestBlackScholesIntegration:
    ...
    def test_moneyness_relationship(self) -> None:
        ...
        # ITM (In The Money)
        itm_price = models.call_price(110.0, 100.0, t, r, sigma)

        # ATM (At The Money)
        atm_price = models.call_price(100.0, 100.0, t, r, sigma)

        # OTM (Out of The Money)
        otm_price = models.call_price(90.0, 100.0, t, r, sigma)
    ...
    def test_time_decay(self) -> None:
        ...
        prices = [models.call_price(s, k, t, r, sigma) for t in times]
    ...
    def test_volatility_impact(self) -> None:
        ...
        prices = [models.call_price(s, k, t, r, vol) for vol in vols]
    ...
    def test_interest_rate_impact(self) -> None:
        ...
        prices = [models.call_price(s, k, t, r, sigma) for r in rates]
    ...
    def test_deep_itm_otm(self) -> None:
        ...
        deep_itm = models.call_price(200.0, 50.0, t, r, sigma)
        intrinsic = 200.0 - 50.0 * np.exp(-r * t)
        assert abs(deep_itm - intrinsic) < 0.01, "Deep ITMが本質的価値に収束していない"

        # Deep OTM: 価格 ≈ 0
        deep_otm = models.call_price(50.0, 200.0, t, r, sigma)
    ...
    def test_batch_vs_single_consistency(self) -> None:
        ...
        # バッチ処理
        batch_prices = models.call_price_batch(spots, k, t, r, sigma)

        # 単一処理との比較（サンプリング）
        sample_indices = np.random.choice(n, 100, replace=False)
        for idx in sample_indices:
            single_price = models.call_price(spots[idx], k, t, r, sigma)
    ...
    def test_price_continuity(self) -> None:
        ...
        prices = [models.call_price(s, k, t, r, sigma) for s in spots]
    ...
    def test_term_structure(self) -> None:
        ...
        prices = [models.call_price(s, k, t, r, sigma) for t in terms]
    ...
    def test_volatility_smile_preparation(self) -> None:
        ...
        for k in strikes:
            # 現在は固定ボラティリティ
            price = models.call_price(s, k, t, r, base_vol)
            prices.append(price)
...
class TestBlackScholesAccuracy:
    ...
    def test_greeks_finite_difference(self) -> None:
        ...
        base_price = models.call_price(s, k, t, r, sigma)
        ...
        # ベガ（∂C/∂σ）
        dv = 0.001
        price_vol_up = models.call_price(s, k, t, r, sigma + dv)
        vega = (price_vol_up - base_price) / dv
        assert vega > 0, f"ベガが負: {vega}"

        # シータ（-∂C/∂T）
        dt = 0.01
        price_time_down = models.call_price(s, k, t - dt, r, sigma)
        ...
        # ロー（∂C/∂r）
        dr = 0.001
        price_rate_up = models.call_price(s, k, t, r + dr, sigma)
...
class TestMarketScenarios:
    ...
    def test_fx_options(self) -> None:
        ...
        price = models.call_price(s, k, t, r_adjusted, v)
