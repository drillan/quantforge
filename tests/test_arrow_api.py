...
class TestArrowAPI:
    ...
    def test_implied_volatility_arrow(self):
        ...
        iv = qf_arrow.implied_volatility(
            price, spots, strikes, times, rates, is_call=True
        )
...
class TestNumPyCompat:
    ...
    def test_numpy_arrow_consistency(self):
        ...
        arrow_result = qf_arrow.call_price(
            pa.array(spots), pa.array(strikes), pa.array(times),
            pa.array(rates), pa.array(sigmas)
        )
        
        # Should be identical
        np.testing.assert_array_almost_equal(
            numpy_result, arrow_result.to_numpy(), decimal=10
        )
...
class TestPerformanceComparison:
    ...
    def test_arrow_vs_numpy_consistency(self):
        ...
        numpy_direct = black_scholes.call_price_batch(
        ...
        # NumPy compat
        numpy_compat = qf_numpy.call_price_numpy(
            spots, strikes, times, rates, sigmas
        )
        
        # All should be identical
        np.testing.assert_array_almost_equal(
            numpy_direct, arrow_result.to_numpy(), decimal=10
        )
        np.testing.assert_array_almost_equal(
            numpy_direct, numpy_compat, decimal=10
        )