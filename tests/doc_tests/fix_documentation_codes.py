...
class CodeFix:
    ...
    replacement: str
    description: str
    context: Optional[str] = None  # 特定のコンテキストでのみ適用
...
class DocumentationCodeFixer:
    ...
    BATCH_API_PARAM_FIXES = [
        CodeFix(
            pattern=r'\bk=',
            replacement='strikes=',
            description='バッチAPIのstrikeパラメータ修正',
            context='_batch'
        ),
        CodeFix(
            pattern=r'\bt=',
            replacement='times=',
            description='バッチAPIのtimeパラメータ修正',
            context='_batch'
        ),
        CodeFix(
            pattern=r'\br=',
            replacement='rates=',
            description='バッチAPIのrateパラメータ修正',
            context='_batch'
        ),
        CodeFix(
            pattern=r'\bsigma=',
            replacement='sigmas=',
            description='バッチAPIのsigmaパラメータ修正',
            context='_batch'
        ),
        CodeFix(
            pattern=r'\bq=',
    ...
    GREEKS_ACCESS_FIXES = [
        CodeFix(
            pattern=r'greeks\.delta\b',
            replacement="greeks['delta']",
            description='Greeks deltaアクセス修正'
        ),
        CodeFix(
            pattern=r'greeks\.gamma\b',
            replacement="greeks['gamma']",
            description='Greeks gammaアクセス修正'
        ),
        CodeFix(
            pattern=r'greeks\.vega\b',
            replacement="greeks['vega']",
            description='Greeks vegaアクセス修正'
        ),
        CodeFix(
            pattern=r'greeks\.theta\b',
            replacement="greeks['theta']",
            description='Greeks thetaアクセス修正'
        ),
        CodeFix(
            pattern=r'greeks\.rho\b',
            replacement="greeks['rho']",
            description='Greeks rhoアクセス修正'
        ),
        CodeFix(
    ...
    GREEKS_BATCH_FIXES = [
        ...
        CodeFix(
            ...
            replacement=r'black76.greeks_batch(\n    forwards=\1,\n    strikes=\2,\n    times=\3,\n    rates=\4,\n    sigmas=\5,\n    is_calls=\6\n)',
    ...
    def _fix_block_types(self, content: str) -> Tuple[str, int]:
        ...
        def fix_block_type(match):
            ...
            if check_if_bash(code):
                fix_count += 1
                if self.verbose:
                    print(f"  Changed python to bash block")
                return f"```bash\n{code}\n```"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ドキュメント内のPythonコードサンプルを自動修正'
    )
    parser.add_argument(
        'path',
        type=Path,
        help='修正対象のファイルまたはディレクトリ'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='実際にファイルを変更せず、変更内容を表示'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
