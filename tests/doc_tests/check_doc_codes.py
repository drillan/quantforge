...
def main():
    ...
    # 統計情報
    stats = defaultdict(lambda: {'total': 0, 'test': 0, 'skip': 0, 'pass': 0, 'fail': 0})
    errors = []
    ...
    # サマリー
    print("\n## サマリー\n")
    print(f"✓ 成功率: {total_pass}/{total_test} ({100*total_pass/total_test:.1f}%)" if total_test > 0 else "テスト対象なし")
