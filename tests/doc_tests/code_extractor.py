...
try:
    ...
    nodes = None
    warnings.warn(
        "myst-parser not available, falling back to basic regex extraction",
        stacklevel=2
    )
...
class EnrichedCodeBlock:
    ...
    ast_node: Any | None = None  # 元のASTノード
...
class MystDocumentCodeExtractor:
    ...
    def __init__(self):
        ...
        if not MYST_AVAILABLE:
            raise ImportError(
                "myst-parser is required for advanced code extraction. Install with: pip install myst-parser"
            )
    ...
    def extract_from_file(self, file_path: Path) -> list[EnrichedCodeBlock]:
        ...
        for node in doc.findall(getattr(nodes, "literal_block", None)):
            if enriched_block := self._create_enriched_block(node, file_path, document_structure):
    ...
    def _extract_section_title(self, section_node) -> str | None:
        """セクションのタイトルを抽出。"""
    ...
    def _create_enriched_block(
        self, node, file_path: Path, document_structure: dict[str, Any]
    ) -> EnrichedCodeBlock | None:
        """完全な文脈情報付きコードブロック作成。"""
    ...
    def _should_skip_intelligent(self, code: str, execution_context: str, section_path: list[str]) -> bool:
        """完全に知的な自動スキップ判定。"""
        ...
        return any(re.search(pattern, code, re.MULTILINE) for pattern in skip_patterns)
