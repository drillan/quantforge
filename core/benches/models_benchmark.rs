...
use quantforge_core::models::{american::American, black76::Black76, black_scholes::BlackScholes, merton::Merton};
...
fn bench_american(c: &mut Criterion) {
    ...
    group.bench_function("exercise_boundary_call", |b| {
        let params = AmericanParams { s, k, t, r, q, sigma };
        b.iter(|| {
            black_box(calculate_exercise_boundary(&params, true))
        })
    });
    ...
    group.bench_function("exercise_boundary_put", |b| {
        let params = AmericanParams { s, k, t, r, q, sigma };
        b.iter(|| {
            black_box(calculate_exercise_boundary(&params, false))
        })
    });
...
fn bench_batch_processing(c: &mut Criterion) {
    ...
    for size in &[100, 1000, 10000] {
        ...
        group.bench_function(format!("sequential_{n}"), |b| {
            b.iter(|| {
                for i in 0..n {
                    let _ = black_box(model.call_price(
                        spots[i],
                        strikes[i],
                        times[i],
                        rates[i],
        ...
        group.bench_function(format!("parallel_{n}"), |b| {
            b.iter(|| {
                black_box(model.call_price_batch(
                    &spots,
                    &strikes,
                    &times,
                    &rates,
