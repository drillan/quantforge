...
/// 正規累積分布関数（norm_cdf）の上限
///
/// この値を超えると確率は実質的に1.0。
/// Φ(8) ≈ 0.9999999999999997
pub const NORM_CDF_UPPER_BOUND: f64 = 8.0;

/// 正規累積分布関数（norm_cdf）の下限
///
/// この値を下回ると確率は実質的に0.0。
/// Φ(-8) ≈ 0.0000000000000003
pub const NORM_CDF_LOWER_BOUND: f64 = -8.0;

/// 正規確率密度関数（norm_pdf）のゼロ境界
///
/// この絶対値を超えると密度は実質的に0.0。
/// pdf(±40) ≈ 1e-350（アンダーフロー）
