    ...
    let results = py.allow_threads(move || {
        ...
        if iter.len() < PARALLEL_THRESHOLD_SMALL {
            ...
            iter.compute_with(|values| {
                Merton::call_price_merton(values[0], values[1], values[2], values[3], values[4], values[5])
                    .unwrap_or(f64::NAN)
            })
        } else {
...
fn put_price_batch<'py>(
    ...
    let results = py.allow_threads(move || {
        ...
        if iter.len() < PARALLEL_THRESHOLD_SMALL {
            // Sequential processing for small data
            iter.compute_with(|values| {
                Merton::put_price_merton(values[0], values[1], values[2], values[3], values[4], values[5])
                    .unwrap_or(f64::NAN)
            })
        } else {
            // Parallel processing for large data
        ...
        py.allow_threads(move || {
            ...
            if len < PARALLEL_THRESHOLD_SMALL {
                ...
                for i in 0..len {
                    ...
                    let greeks = Merton::greeks_merton(s, k, t, r, q, sigma, is_call)
                        .unwrap_or(Greeks {
                            delta: f64::NAN,
                            gamma: f64::NAN,
                            vega: f64::NAN,
                ...
                let greeks_results: Vec<_> = (0..len)
                    ...
                    .map(|i| {
                        ...
                        Merton::greeks_merton(s, k, t, r, q, sigma, is_call)
                            .unwrap_or(Greeks {
                                delta: f64::NAN,
                                gamma: f64::NAN,
                                vega: f64::NAN,
