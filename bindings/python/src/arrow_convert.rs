...
pub fn arrow_to_numpy<'py>(
    py: Python<'py>, 
    arr: &Float64Array
) -> Bound<'py, PyArray1<f64>> {
    // Extract values from Arrow array
    let vec: Vec<f64> = (0..arr.len())
        .map(|i| arr.value(i))
        .collect();
...
pub fn arrayref_to_numpy<'py>(
    ...
    let float_array = arr
        ...
        .ok_or_else(|| {
            pyo3::exceptions::PyTypeError::new_err("Expected Float64Array")
        })?;
...
pub fn broadcast_to_length(
    ...
    let arr_len = arr.len()
        .map_err(|e| ArrowError::InvalidArgumentError(format!("Failed to get array length: {}", e)))?;
    ...
    if arr_len == 1 {
        ...
        Err(ArrowError::InvalidArgumentError(
            format!("Cannot broadcast array of length {} to length {}", arr_len, target_len)
        ))
    }
}
