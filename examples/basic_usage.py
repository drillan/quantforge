...
def demonstrate_single_calculation() -> None:
    ...
    price = quantforge.black_scholes.call_price(s, k, t, r, v)
    print(f"   スポット価格: ${s:.2f}")
    print(f"   権利行使価格: ${k:.2f}")
    print(f"   満期: {t:.1f}年")
    print(f"   金利: {r:.1%}")
...
def demonstrate_batch_calculation() -> None:
    ...
    prices = quantforge.black_scholes.call_price_batch(spots, k, t, r, v)
...
def run_performance_test() -> None:
    ...
    for _ in range(n_single):
        quantforge.black_scholes.call_price(100.0, 100.0, 1.0, 0.05, 0.2)
    ...
    start = time.perf_counter()
    _ = quantforge.black_scholes.call_price_batch(spots_large, 100.0, 1.0, 0.05, 0.2)
