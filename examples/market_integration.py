...
def process_market_data(df, spot, rate):
    ...
    for expiry_days, group in df.groupby('expiry_days'):
        ...
        weighted_mids = weighted_mid_price_batch(bids, asks, bid_vols, ask_vols)
...
def calculate_portfolio_risk(df, spot, rate, portfolio_positions):
    ...
    for i, (strike, row) in enumerate(df_positions.iterrows()):
        ...
        print(f"{row['strike']:6.1f} | {row['type']:4s} | {qty:5d} | "
