...
fn calculate_call_price(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    validate_inputs(s, k, t, r, sigma)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;

    Ok(black_scholes::bs_call_price(s, k, t, r, sigma))
}
...
fn calculate_call_price_batch<'py>(
    ...
    let results = if spots.len() >= 30000 {
        black_scholes_parallel::bs_call_price_batch_parallel(spots, k, t, r, sigma)
    } else {
        black_scholes::bs_call_price_batch(spots, k, t, r, sigma)
    };
...
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_put_price(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    validate_inputs(s, k, t, r, sigma)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;

    Ok(black_scholes::bs_put_price(s, k, t, r, sigma))
}
...
fn calculate_put_price_batch<'py>(
    ...
    // 大規模データは並列処理を使用（30000要素以上で並列化が効果的）
    let results = if spots.len() >= 30000 {
        black_scholes_parallel::bs_put_price_batch_parallel(spots, k, t, r, sigma)
    } else {
        black_scholes::bs_put_price_batch(spots, k, t, r, sigma)
    };
    Ok(PyArray1::from_vec_bound(py, results))
}
...
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_delta_call(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    ...
    Ok(greeks::delta_call(s, k, t, r, sigma))
}

/// Python向け: Delta Put計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_delta_put(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    // 満期時（t=0）は特別処理されるのでバリデーションをスキップ
    if t > 0.0 {
        validate_inputs(s, k, t, r, sigma)
            .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    Ok(greeks::delta_put(s, k, t, r, sigma))
}

/// Python向け: Gamma計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_gamma(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    // 満期時（t=0）は特別処理されるのでバリデーションをスキップ
    if t > 0.0 {
        validate_inputs(s, k, t, r, sigma)
            .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    Ok(greeks::gamma(s, k, t, r, sigma))
}

/// Python向け: Vega計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_vega(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    // 満期時（t=0）は特別処理されるのでバリデーションをスキップ
    if t > 0.0 {
        validate_inputs(s, k, t, r, sigma)
            .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    Ok(greeks::vega(s, k, t, r, sigma))
}

/// Python向け: Theta Call計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_theta_call(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    // 満期時（t=0）は特別処理されるのでバリデーションをスキップ
    if t > 0.0 {
        validate_inputs(s, k, t, r, sigma)
            .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    Ok(greeks::theta_call(s, k, t, r, sigma))
}

/// Python向け: Theta Put計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_theta_put(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    // 満期時（t=0）は特別処理されるのでバリデーションをスキップ
    if t > 0.0 {
        validate_inputs(s, k, t, r, sigma)
            .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    Ok(greeks::theta_put(s, k, t, r, sigma))
}

/// Python向け: Rho Call計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_rho_call(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    // 満期時（t=0）は特別処理されるのでバリデーションをスキップ
    if t > 0.0 {
        validate_inputs(s, k, t, r, sigma)
            .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    Ok(greeks::rho_call(s, k, t, r, sigma))
}

/// Python向け: Rho Put計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma))]
fn calculate_rho_put(s: f64, k: f64, t: f64, r: f64, sigma: f64) -> PyResult<f64> {
    // 満期時（t=0）は特別処理されるのでバリデーションをスキップ
    if t > 0.0 {
        validate_inputs(s, k, t, r, sigma)
            .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    ...
    Ok(greeks::rho_put(s, k, t, r, sigma))
}

/// Python向け: 全グリークス計算
#[pyfunction]
#[pyo3(signature = (s, k, t, r, sigma, is_call))]
fn calculate_all_greeks(
    ...
    let greeks = greeks::calculate_all_greeks(s, k, t, r, sigma, is_call);
...
fn calculate_delta_call_batch<'py>(
    ...
    // 大規模データは並列処理を使用
    let results = if spots.len() >= 30000 {
        greeks_parallel::delta_call_batch_parallel(spots, k, t, r, sigma)
    } else {
        greeks::delta_call_batch(spots, k, t, r, sigma)
    };
    Ok(PyArray1::from_vec_bound(py, results))
}
...
fn calculate_gamma_batch<'py>(
    ...
    // 大規模データは並列処理を使用
    let results = if spots.len() >= 30000 {
        greeks_parallel::gamma_batch_parallel(spots, k, t, r, sigma)
    } else {
        greeks::gamma_batch(spots, k, t, r, sigma)
    };
    Ok(PyArray1::from_vec_bound(py, results))
}
