...
impl PricingModel for BlackScholes {
    ...
    fn call_price(params: &Self::Params) -> f64 {
        ...
        let d1 = (params.spot.ln() - params.strike.ln() + (params.rate + params.sigma * params.sigma / 2.0) * params.time) 
            / (params.sigma * sqrt_t);
        let d2 = d1 - params.sigma * sqrt_t;
        
        // Prevent negative values for deep OTM due to numerical errors
        (params.spot * norm_cdf(d1) - params.strike * (-params.rate * params.time).exp() * norm_cdf(d2)).max(0.0)
    }
    ...
    fn put_price(params: &Self::Params) -> f64 {
        ...
        // Prevent negative values for deep OTM due to numerical errors
        (params.strike * (-params.rate * params.time).exp() * norm_cdf(-d2) - params.spot * norm_cdf(-d1)).max(0.0)
    }
