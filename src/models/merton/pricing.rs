...
/// Batch calculation of call prices for multiple spot prices.
///
/// # Arguments
...
pub fn call_price_batch(spots: &[f64], k: f64, t: f64, r: f64, q: f64, sigma: f64) -> Vec<f64> {
    ...
    if t <= 0.0 {
        return spots.iter().map(|&s| (s - k).max(0.0)).collect();
    }
    ...
    spots
        ...
        .map(|&s| {
            ...
            let d1 = (s.ln() - k_ln + (r - q + half_sigma_squared) * t) / sigma_sqrt_t;
...
/// Batch calculation of put prices for multiple spot prices.
///
/// # Arguments
/// * `spots` - Array of spot prices
/// * `k` - Strike price
...
pub fn put_price_batch(spots: &[f64], k: f64, t: f64, r: f64, q: f64, sigma: f64) -> Vec<f64> {
    // Pre-calculate common factors for efficiency
    if t <= 0.0 {
        return spots.iter().map(|&s| (k - s).max(0.0)).collect();
    }
...
/// Batch calculation of call prices for multiple dividend yields.
///
/// # Arguments
/// * `s` - Spot price
/// * `k` - Strike price
/// * `t` - Time to maturity (years)
/// * `r` - Risk-free rate
/// * `qs` - Array of dividend yields
/// * `sigma` - Volatility
pub fn call_price_batch_q(s: f64, k: f64, t: f64, r: f64, qs: &[f64], sigma: f64) -> Vec<f64> {
    // Pre-calculate common factors for efficiency
    if t <= 0.0 {
        return vec![(s - k).max(0.0); qs.len()];
    }
    ...
    qs.iter()
        .map(|&q| {
            let d1 = (s_ln - k_ln + (r - q + half_sigma_squared) * t) / sigma_sqrt_t;
            let d2 = d1 - sigma_sqrt_t;
            let exp_neg_qt = (-q * t).exp();
...
/// Batch calculation of put prices for multiple dividend yields.
///
/// # Arguments
/// * `s` - Spot price
/// * `k` - Strike price
...
/// * `qs` - Array of dividend yields
/// * `sigma` - Volatility
pub fn put_price_batch_q(s: f64, k: f64, t: f64, r: f64, qs: &[f64], sigma: f64) -> Vec<f64> {
    // Pre-calculate common factors for efficiency
    if t <= 0.0 {
        return vec![(k - s).max(0.0); qs.len()];
    }

    if s <= 0.0 {
        let exp_neg_rt = (-r * t).exp();
        return vec![k * exp_neg_rt; qs.len()];
    }
